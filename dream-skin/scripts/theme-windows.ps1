...
function Initialize-DreamSkinThemeStore {
  ...
  $assetImage = Join-Path $assetRoot 'open-source-demo-background.png'
  ...
  if (-not (Test-Path -LiteralPath $activeTheme -PathType Leaf)) {
    ...
    Assert-DreamSkinNoReparseComponents -Path (Join-Path $paths.Active 'open-source-demo-background.png')
    $activeImage = Join-Path $paths.Active 'open-source-demo-background.png'
    Copy-Item -LiteralPath (Join-Path $assetRoot 'open-source-demo-background.png') `
    ...
    $imageArchive = Join-Path $paths.Images 'open-source-demo-background.png'
  ...
  # Bundled neutral demo preset (the image lives under assets/).
  $presetDirectory = Join-Path $paths.Saved 'open-source-aurora-demo'
  ...
  if (-not (Test-Path -LiteralPath $presetTheme -PathType Leaf)) {
    ...
    $presetImage = Join-Path $presetDirectory 'open-source-demo-background.png'
...
function Import-DreamSkinSavedTheme {
  param(
  ...
  if (-not $trimmedName -or $trimmedName.Length -gt 80 -or $trimmedName -match '[\u0000-\u001f]') {
  ...
  $staging = Join-Path $paths.Saved ('.importing-' + [guid]::NewGuid().ToString('N'))
  ...
  try {
    ...
    $target = Join-Path $staging $imageName
    ...
    if ($null -eq $Theme) {
      ...
      $Theme = $Theme | ConvertTo-Json -Depth 8 | ConvertFrom-Json
      ...
      $Theme | Add-Member -NotePropertyName name -NotePropertyValue $trimmedName -Force
      $Theme | Add-Member -NotePropertyName image -NotePropertyValue $imageName -Force
    }
    Write-DreamSkinTheme -ThemeDirectory $staging -Theme $Theme
    ...
    if ((Test-Path -LiteralPath $staging) -and
      (Test-DreamSkinThemePathWithin -Path $staging -Root $paths.Saved)) {
