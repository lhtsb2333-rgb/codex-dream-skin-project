...
function Get-DreamSkinRuntimeEnginePaths {
  ...
  return [pscustomobject]@{
    ...
    UpdateWatcher = Join-Path $scripts 'watch-dream-skin-updates.ps1'
  }
}
...
function Install-DreamSkinRuntimeEngine {
  ...
  $required = @(
    'assets\open-source-demo-background.png',
    'assets\dream-skin.css',
    'assets\renderer-inject.js',
    'assets\theme.json',
    'scripts\common-windows.ps1',
    'scripts\config-utf8.ps1',
    'scripts\diagnose-shell.mjs',
    'scripts\image-metadata.mjs',
    'scripts\injector.mjs',
    'scripts\install-dream-skin.ps1',
    'scripts\restore-dream-skin.ps1',
    'scripts\set-window-icon.ps1',
    'scripts\start-dream-skin.ps1',
    'scripts\theme-windows.ps1',
    'scripts\tray-dream-skin.ps1',
    'scripts\verify-dream-skin.ps1',
    'scripts\watch-dream-skin-updates.ps1',
    'scripts\uninstall-dream-skin-manager.ps1',
    'assets\manager-version-1.4.8.txt'
  )
  $runtimeDirectories = @('assets', 'scripts', 'presets')
  if (Test-Path -LiteralPath (Join-Path $sourceRoot 'runtime\node.exe') -PathType Leaf) {
    $required += @('runtime\node.exe', 'runtime\LICENSE-node.txt')
...
function Get-DreamSkinNodeRuntime {
  ...
  $bundledNode = Join-Path (Split-Path -Parent $PSScriptRoot) 'runtime\node.exe'
  $runtimeCandidate = $null
  if (Test-Path -LiteralPath $bundledNode -PathType Leaf) {
    $runtimeCandidate = $bundledNode
  } else {
  ...
  if (-not $runtimeCandidate) {
    throw "Node.js $MinimumMajor or newer is required; the bundled runtime and PATH runtime were not found."
  }
  $versionProbe = Invoke-DreamSkinNative -FilePath $runtimeCandidate -ArgumentList @('-p', 'process.versions.node') -DiscardStderr
  ...
  $pathProbe = Invoke-DreamSkinNative -FilePath $runtimeCandidate -ArgumentList @('-p', 'process.execPath') -DiscardStderr
