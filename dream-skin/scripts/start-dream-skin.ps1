...
try {
  ...
  if ($ProfilePath) {
    ...
    # directory. Keep a dedicated persistent profile instead of copying or
    # weakening the protection of Codex's official profile.
    $ProfilePath = Join-Path $StateRoot 'codex-skin-profile'
  ...
  # Apply the bundled icon only to windows owned by the validated Codex package.
  # This is best-effort because some Windows builds retain the packaged-app icon.
  try {
    $iconHelper = Join-Path $PSScriptRoot 'set-window-icon.ps1'
    $windowIcon = Join-Path (Split-Path -Parent $PSScriptRoot) 'assets\karyl-window.ico'
    if ((Test-Path -LiteralPath $iconHelper -PathType Leaf) -and
      (Test-Path -LiteralPath $windowIcon -PathType Leaf)) {
      $powershell = (Get-Command powershell.exe -ErrorAction Stop).Source
      $iconArguments = "-NoProfile -ExecutionPolicy RemoteSigned -File $(ConvertTo-DreamSkinProcessArgument -Value $iconHelper)" +
        " -CodexExe $(ConvertTo-DreamSkinProcessArgument -Value $codex.Executable)" +
        " -IconPath $(ConvertTo-DreamSkinProcessArgument -Value $windowIcon)"
      Start-Process -FilePath $powershell -ArgumentList $iconArguments -WindowStyle Hidden | Out-Null
    }
  } catch {
    Write-Warning "Codex started, but its taskbar icon could not be updated: $($_.Exception.Message)"
  }
  ...
  try {
    ...
    Remove-Item -LiteralPath (Join-Path $StateRoot 'start-error.log') -Force -ErrorAction SilentlyContinue
  } catch {
    $startupError = $_
  ...
  $errorRecord = $_
  try {
    $failureRoot = Join-Path $env:LOCALAPPDATA 'CodexDreamSkin'
    ...
    [System.IO.File]::WriteAllText(
      (Join-Path $failureRoot 'start-error.log'),
      $failureText + "`r`n",
