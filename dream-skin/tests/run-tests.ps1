...
try {
  ...
  foreach ($requiredCss in @(
    ...
    '.dream-home [class*="_homeUtilityBar_"]',
    ':is(.dream-task-ambient, .dream-task-banner):has(main.dream-thread-shell)',
    'main.dream-thread-shell',
    '.dream-thread'
  )) {
    if (-not $css.Contains($requiredCss)) { throw "Windows immersive CSS is missing: $requiredCss" }
  }
  if ($css -match 'html\.codex-dream-skin main\s*\{[^}]*\b(?:position|overflow|isolation|min-height)\s*:') {
    throw 'The skin must not override Codex main-shell layout or clipping properties.'
  }
  if ($css -match 'dream-home-utility|\.dream-home:has\(\.dream-home-utility\)') {
    throw 'The home utility bar must retain native geometry rather than being merged with the composer.'
  }
  ...
  $watcherSource = Read-DreamSkinUtf8File -Path (Join-Path $Root 'scripts\watch-dream-skin-updates.ps1')
  foreach ($requiredWatcherAction in @('Open-DreamSkinManager', 'Add_DoubleClick', '打开主题管理器', 'CODEX_DREAM_SKIN_LAUNCHER')) {
    if (-not $watcherSource.Contains($requiredWatcherAction)) { throw "Watcher tray action is missing: $requiredWatcherAction" }
  }
  ...
  foreach ($requiredRendererBehavior in @('dream-home-utility', 'dream-thread-shell', '.thread-scroll-container', '[data-message-author-role]', 'artMetadata', 'detectShellAppearance')) {
  ...
  $diagnosticSource = Read-DreamSkinUtf8File -Path (Join-Path $Root 'scripts\diagnose-shell.mjs')
  foreach ($requiredDiagnosticBehavior in @(
    'compatibleTargetCount', 'semanticMainElement', 'legacyShellClass',
    'home-or-empty-project', 'compatibleIdentity'
  )) {
    if (-not $diagnosticSource.Contains($requiredDiagnosticBehavior)) {
      throw "Shell diagnostic is missing: $requiredDiagnosticBehavior"
    }
  }
  $playbook = Read-DreamSkinUtf8File -Path (Join-Path $Root 'MAINTENANCE_PLAYBOOK.md')
  foreach ($requiredMethodology in @(
    '安装发现层', '主窗口身份层', '页面状态层', '生命周期层',
    '最低测试矩阵', '不要用哈希类名'
  )) {
    if (-not $playbook.Contains($requiredMethodology)) {
      throw "Maintenance playbook is incomplete: $requiredMethodology"
    }
  }
  $injectorSource = Read-DreamSkinUtf8File -Path (Join-Path $Root 'scripts\injector.mjs')
