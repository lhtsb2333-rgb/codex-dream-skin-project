...
const featured = await fs.readFile(path.join(windowsRoot, "assets", "open-source-demo-background.png"));
const helper = path.join(windowsRoot, "scripts", "image-metadata.mjs");

const metadata = readImageMetadata(featured, ".png");
assert.ok(metadata && metadata.width > 0 && metadata.height > 0);
assert.equal(metadata.wide, true);

const cli = spawnSync(process.execPath, [helper, "--check", path.join(windowsRoot, "assets", "open-source-demo-background.png")], {
  encoding: "utf8",
});
assert.equal(cli.status, 0);
assert.deepEqual(JSON.parse(cli.stdout), metadata);
