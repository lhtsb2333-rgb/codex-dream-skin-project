...
function createFixture() {
  ...
  let nextTimer = 1;
  const markers = { legacyShell: false, semanticShell: false, sidebar: false };
  const context = {
    window: { installs: [] },
    document: {
      documentElement: {},
      ...
      querySelector(selector) {
        if (selector === "main.main-surface") return markers.legacyShell ? {} : null;
        if (selector === "main") return markers.semanticShell ? {} : null;
...
guarded.markers.semanticShell = true;
