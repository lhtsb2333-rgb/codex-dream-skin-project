((cssText, artDataUrl, rawConfig) => {
  ...
  const clearSkinDom = () => {
    ...
    document.querySelectorAll(".dream-thread").forEach((node) => node.classList.remove("dream-thread"));
    document.querySelectorAll(".dream-home-shell").forEach((node) => node.classList.remove("dream-home-shell"));
    document.querySelectorAll(".dream-thread-shell").forEach((node) => node.classList.remove("dream-thread-shell"));
  ...
  const ensure = () => {
    ...
    const hasThreadMarker = (candidate) => Boolean(
      candidate.matches?.(".thread-scroll-container") ||
      candidate.querySelector?.(
        ".thread-scroll-container, [data-thread-scroll-footer], [data-message-author-role]"
      )
    );
    const hasHomeMarker = (candidate) => !hasThreadMarker(candidate) && Boolean(
      candidate.matches?.('[class*="container-name:home-main-content"]') ||
      candidate.querySelector?.(
        '[data-testid="home-icon"], [class~="group/home-suggestions"], [class*="_homeUtilityBar_"]'
      )
    );
    const home = mainCandidates.find(hasHomeMarker) ?? null;
    let hasThread = false;
    // The shell survives React route changes. Clear its old route classes before
    // marking the current route so a new-chat page can never inherit thread CSS.
    if (!mainCandidates.includes(shellMain)) {
      shellMain.classList.remove("dream-home", "dream-task", "dream-thread");
    }
    for (const candidate of mainCandidates) {
      const isHome = candidate === home;
      const isThread = !isHome && hasThreadMarker(candidate);
      candidate.classList.toggle("dream-home", isHome);
      candidate.classList.toggle("dream-task", !isHome);
      candidate.classList.toggle("dream-thread", isThread);
      hasThread ||= isThread;
    }
    ...
    shellMain.classList.toggle("dream-thread-shell", hasThread);
  ...
  window[STATE_KEY] = {
    ensure, cleanup, observer, timer, scheduler, artUrl, profile, config, installToken, version: "1.4.8",
  };
  ensure();
  ...
  return { installed: true, version: "1.4.8", adaptive: true };
