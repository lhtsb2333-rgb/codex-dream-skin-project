...
- Manager 1.3.7 replaces the executable `.lnk.disabled` startup-folder sentinel with a non-executable `update-watcher.disabled` marker in the Dream Skin state directory.
- Existing disabled startup shortcuts are migrated out of the Windows Startup folder so Windows cannot attempt to launch them at sign-in.
- Manager 1.3.6 restores CDP startup on Chromium 136+ by using a dedicated persistent Dream Skin user-data directory instead of the protected default Codex profile.
- Startup failures are now persisted to `start-error.log` even when the launcher is windowless.
- A disabled `Codex Dream Skin 更新守护.lnk.disabled` login entry is now a persistent opt-out respected by repairs, theme application, the graphical installer, and the watcher itself.
- The watcher suppresses restart prompts for 30 minutes after a recent skin startup failure and records failed restart attempts instead of entering a process-key restart loop.
- Manager 1.3.5 gives the persistent update-watcher tray icon a responsive context menu with a complete-exit command.
- The watcher pumps Windows Forms messages while idle and disposes its tray icon and menu before its process exits.
- The skinned-launch shortcut now always uses the runtime embedded in the current portable EXE instead of a potentially stale installed-engine copy after official Codex updates.
- Manager 1.3.4 imports images into the theme collection without changing the active theme or restarting Codex.
- Image import now accepts a file picker, drag-and-drop, pasted image files, and pasted bitmap data; unsupported preview decoders no longer turn a successful import into an error.
- Installation status is detected before the window is shown and distinguishes installed, update-available, and not-installed states.
- Manager 1.3.3 adds an extensible settings window and a persisted option controlling whether every ordinary Codex launch should be checked for Dream Skin mode.
- With per-launch checking disabled, the update watcher prompts only after detecting an official Codex package version change.
- Manager 1.3.2 ships both the portable ZIP and a graphical installer with a selectable destination, progress display, verified extraction, upgrade rollback, and optional shortcuts.
- Installer upgrades are restricted to empty directories or validated Codex Dream Skin product folders.
- Manager 1.3.1 adds a dedicated root-level uninstaller that restores only Dream Skin-managed Codex appearance keys and removes the manager's own state, themes, caches, shortcuts, login watcher, and portable folder.
- Uninstall deletion is restricted to exact validated application directories and matching shortcuts; reparse points and broad system or user folders are rejected.
- Manager 1.3.0 adds a per-user update watcher that detects Store package version changes, stale skin sessions, and repeated compatibility failures.
- The watcher starts at Windows sign-in without a console window, prompts before restarting Codex, and can safely return to the official appearance after two failed health checks.
- Manager 1.2.5 assigns the PowerShell-hosted manager window an explicit Windows application identity so its running taskbar button retains the bundled Karyl icon.
- Manager 1.2.4 preserves Codex's native `fixed` and `sticky` positioning for route toolbars.
- Fix the header being shifted right by exactly the expanded sidebar width, which pushed pane controls off-screen.

- Manager 1.2.3 stops clipping the native main shell so review/source-pane collapse controls remain visible.
- Keep the native app header above the artwork layer without changing its geometry or interactions.

- Manager 1.2.2 launches the skin shortcut through the windowless EXE mode instead of a visible PowerShell console.
- Desktop, Start menu, and existing pinned-taskbar shortcuts are rewritten to the stable portable EXE target during each repair or update.

- Manager 1.2.1 exposes only the bundled dual-character theme and names it `GBFR&PCR`.
- Runtime upgrade removes the three retired bundled themes while preserving user-imported themes.

- Apply the bundled Karyl icon to the running Codex window and taskbar on a best-effort basis.
- Add safe rename and delete controls for imported themes.
- Protect bundled themes, the active theme, and the last saved theme from deletion.
